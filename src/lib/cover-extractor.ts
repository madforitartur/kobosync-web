...
export async function extractCoverFromEpub(
  ...
  // Estratégia 2: properties="cover-image"
  if (!coverItem) {
...
export async function uploadCoverForBook(
  driveFileId: string,
  bookId: string,
  downloadFromDrive: (id: string) => Promise<ArrayBuffer>,
): Promise<{ url: string; path: string } | null> {
  try {
    const epubBuffer = await downloadFromDrive(driveFileId);
